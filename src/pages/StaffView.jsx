            ...
            <div className="glass-panel sticky top-0 z-30 px-4 py-3 md:px-8 md:py-5 flex items-center justify-between gap-3 border-b border-slate-100/60 shadow-sm transition-all pt-safe-offset-2">
                <div className="flex-1 min-w-0">
                    <h1 className="text-lg md:text-2xl font-bold text-slate-900 tracking-tight flex items-center gap-2 truncate">
                        <ListTodo size={20} className="text-indigo-600 hidden sm:block shrink-0" />
                        My Queue
                    </h1>
                    <p className="text-xs md:text-sm text-slate-500 mt-0.5 truncate hidden sm:block">Manage assignments and track progress.</p>
                </div>
            </div>

            <main className="max-w-4xl mx-auto p-6 md:p-8">
                {/* Filter Tabs */}
                <div className="flex p-1 bg-slate-100/80 rounded-xl mb-6 space-x-1 border border-slate-200/60 w-full sm:w-auto overflow-x-auto no-scrollbar">
                    <button
