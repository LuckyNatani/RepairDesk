    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: '12px 16px 0', background: '#fff', borderBottom: '1px solid #F0F0F0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#F5F5F5', borderRadius: 10, padding: '8px 14px', marginBottom: 10 }}>
          <Search size={16} color="var(--grey-600)" />
          <input value={query} onChange={e => handleQuery(e.target.value)} placeholder="Search tasks…" style={{ flex: 1, border: 'none', background: 'transparent', fontSize: 14, outline: 'none', color: 'var(--grey-900)' }} />
          {query && <button onClick={() => handleQuery('')} style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex' }}><X size={14} color="var(--grey-600)" /></button>}
        </div>
        {/* Filter chips */}
        <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 10, scrollbarWidth: 'none' }}>
          {chips.map(c => (
            <button key={c.id} onClick={() => setFilter(c.id)} style={{ padding: '5px 12px', borderRadius: 100, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap', background: filter === c.id ? 'var(--navy)' : '#F0F0F0', color: filter === c.id ? '#fff' : 'var(--grey-600)', transition: 'all 150ms ease' }}>
              {c.label}
            </button>
          ))}
        </div>
