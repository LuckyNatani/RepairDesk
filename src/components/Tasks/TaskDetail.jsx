    ...
    <div style={{ display: 'flex', gap: 10, padding: '10px 0', borderBottom: '1px solid #F5F5F5' }}>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--grey-900)' }}>{remark.author?.name}</span>
          {remark.is_system && <span className="badge badge-sm" style={{ background: 'var(--grey-100)', color: 'var(--grey-600)' }}>System</span>}
          <span style={{ fontSize: 11, color: 'var(--grey-600)', marginLeft: 'auto' }}>{formatDistanceToNow(new Date(remark.created_at), { addSuffix: true })}</span>
        </div>
        <p style={{ margin: 0, fontSize: 13, color: 'var(--grey-900)', lineHeight: 1.5 }}>{remark.text}</p>
      </div>
    </div>
  )
}
...
function RemarkInput({ taskId, authorId, onSuccess }) {
  ...
  return (
    <div style={{ paddingTop: 12 }}>
      ...
      {charCount >= 450 && <div style={{ fontSize: 11, color: charCount >= 490 ? 'var(--red)' : 'var(--grey-600)', textAlign: 'right', marginBottom: 6 }}>{charCount}/500</div>}
...
export default function TaskDetail({ taskId, currentUser }) {
  ...
  if (!task) return <div style={{ padding: 24, textAlign: 'center', color: 'var(--grey-600)' }}>Task not found</div>
    ...
    <div style={{ maxWidth: 600, margin: '0 auto' }}>
      ...
      <div className="page-content" style={{ padding: '16px' }}>
        ...
        <div className="card" style={{ padding: '14px 16px', marginBottom: 12 }}>
          <h2 style={{ margin: '0 0 8px', fontSize: 20, fontWeight: 700, color: 'var(--grey-900)' }}>{task.customer_name}</h2>
          ...
          {task.customer_address && (
            <a href={mapsUrl} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--grey-600)', fontSize: 13, textDecoration: 'none', marginBottom: 6 }}>
              <MapPin size={14} color="var(--grey-600)" /> {task.customer_address}
            </a>
          )}
          {task.service_description && (
            <p style={{ margin: '8px 0 0', fontSize: 13, color: 'var(--grey-900)', lineHeight: 1.5 }}>{task.service_description}</p>
          )}
          {task.due_at && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 8, color: new Date(task.due_at) < new Date() ? 'var(--red)' : 'var(--grey-600)', fontSize: 12 }}>
        ...
        <div className="card" style={{ padding: '12px 16px', marginBottom: 12, display: 'flex', gap: 16, flexWrap: 'wrap' }}>
          {[['Created', task.created_at], ['Assigned', task.assigned_at], ['Completed', task.completed_at]].map(([label, ts]) => (
            <div key={label}>
              <div style={{ fontSize: 11, color: 'var(--grey-600)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{label}</div>
              <div style={{ fontSize: 12, color: ts ? 'var(--grey-900)' : 'var(--grey-600)', marginTop: 2 }}>{ts ? format(new Date(ts), 'dd MMM, h:mm a') : '—'}</div>
            </div>
          ))}
        </div>
        ...
        <div className="card" style={{ padding: '14px 16px' }}>
          <h3 style={{ margin: '0 0 12px', fontFamily: '"Inter", sans-serif', fontSize: 14, fontWeight: 600, color: 'var(--grey-900)' }}>Remarks & Notes</h3>
          <div ref={remarksRef} id="remarks">
            {remarks.length === 0
              ? <p style={{ color: 'var(--grey-600)', fontSize: 13, margin: 0 }}>No remarks yet.</p>
